...
export default async function VerifyPage({
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm rounded-xl border border-hair bg-panel p-8 shadow-sm">
        {identity ? (
          <AutoSignIn token={token} />
        ) : (
          <div className="text-center">
            <h1 className="text-lg font-semibold text-fg">That link didn&rsquo;t work</h1>
            <p className="mt-2 text-[14px] leading-relaxed text-sub">
              Sign-in links expire after 10 minutes. Request a new one and use the
              most recent email.
            </p>
            <Link
              href="/signin"
